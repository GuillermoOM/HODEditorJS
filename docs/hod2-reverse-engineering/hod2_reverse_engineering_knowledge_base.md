...
* **HOD 1.0 Retrofit Caveats**:
  ...
  * HOD 1.0 inline `LMIP` texture payloads use a different DXT row order than HOD 2.0 pool convention. The `encode_b64_png_thumbnail` function always flips images vertically before PNG encoding (for Three.js `flipY=true` display), which also converts from HOD 1.0's native row order to HOD 2.0's pool convention. At save time, `legacy_storage_y_flipped = true` textures must NOT be un-flipped — the PNG's flipped orientation IS the correct HOD 2.0 pool data. Non-legacy textures (DAE/TGA imports) must be un-flipped to restore their source orientation. The condition in `generate_lmip_texture_chunks_and_pool` is `if !texture.legacy_storage_y_flipped` (flip non-legacy, keep legacy as-is).
