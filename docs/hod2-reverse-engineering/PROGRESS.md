...
## Current Issues
- HOD 1.0 texture visual orientation still needs a manual editor/game retest after the inline LMIP/TEXM offset fix. The parser no longer reads shifted compressed bytes, but the existing `flip_rgba_vertical_in_place` behavior was not changed in this pass.
