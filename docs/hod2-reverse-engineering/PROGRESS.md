...
## Current Status
- ✅ Added Textures subsection to Materials tab (`HierarchyTree.tsx`): The Materials tab in the left panel now contains two sections separated by a divider. The top section lists Materials as before. The bottom section lists all loaded Textures with thumbnail previews, dimensions, and formats. Added right-click context menu options for Textures to either toggle `legacy_storage_y_flipped` or remove the texture entirely.
