...
## Current Issues
- User manual UI confirmation is still pending for `ter_hecate.hod` after the HIER signed-count fix. Parser diagnostics show the model state is correct for the hierarchy tree and NavLight coordinate inspector.
...
## Planned Tasks
- Have the user retest `ter_hecate.hod` in the UI and confirm `Navlight_*` rows appear under `Root`/hardpoints with coordinate editing visible in the NavLight inspector.
